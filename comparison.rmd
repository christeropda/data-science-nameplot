---
title: "Exploring the relationship between birth names and pop culture"
...
This is what lead us to the "simplicity is king" strategy. Display the yearly amount of each name in a graph containing all names, in addition to a static line on the year where the series dropped. We think that this is a good way to show the statistic, because its easy to read and you need no other information than the title of the graphic to understand what point we are trying to make.

Also instead of just analyzing one series, we choose to analyze three different series. Since the data of the names, are specific to Norway and Sweden, we choose to analyze one globally popular series(Game of Thrones), one Norwegian series(SKAM) and one Swedish series(Wallander). Note that the algorithm that were created can infact analyze the relationship between norwegian and swedish baby names to any series contained on the IMDB website. This is possible since all series has the same character name display on the IMDB website,and we planned for it to be like this so we could dynamically switch out the series if we wanted to.

# Initial Questions
When we started the project initially, we were curious if the names that are used for Game of Thrones, were later seen as names for newborns in Norway and Sweden. We also wanted to know if any names were present before the TV show started. However, when we got the results in from Game of Thrones, we realized that few of the names from the series were actually used in reality. From this the logical next step were to look at different TV shows that were made in Norway and Sweden and do the same analysis for those. We then got hit by the issue that for Skam in particular, some of the names were so popular that it became very hard to read the graphs. To mitigate this issue, we limited the amount of episodes that each character played in. Since the popularity of the different names are different from show to show, this limitation is also different between the shows. This is further explained later on. 
...
The data were originally organized as excel files, but we found out that while using python, converting the files to text files were faster and easier to work with.
...
We decided to show the data as a graph, where the number of newborns for each year for each country is shown in different graphs. We did initially want to show all names for one country in the same graph, but the graphs became unreadable because of all the information in them. We also looked into different plot methods, but in the end we came to the conclusion that a graph displays the different changes overtime better than the other charts we did look into. When reading the graphs, since we were interested in any changes after a show started, we added a vertical line to easier display the difference before and after to see if anything changed. The data we have could of course be displayed in a table, with the different numbers in cells next to each other. However, we decided not to go this route as we found the graph to be more readable for changes overtime, which is what we were aiming for. 

When plotting the different statistics it was important to make the data easily readable, and they had to be understandable for bystanders without additional knowledge about the project. There were multiple renditions, but we ultimately decided to show graphs containing multiple names and showing when the series came out in the same graph. To limit the amount of names on each graph we simply limited the amount of episodes the different character names had to be present in the series for them to be "important" enough to be on the graph.This was particularly important for the Norwegian series "Skam" which feature an enormous amount of character names spanning over many episodes. A limit at 10 episodes were shown to be an efficient metric. Note that different limits were used for different series. In addition when looking at the graphs, some might say that there are names missing, meaning that there are some popular name that is not present in the graph. This is either due to the above mentioned limit, or due to the fact that there are no newborns named that specific name.
...
*  There are some evidence that newborn baby names are influenced by pop culture, specifically from tv series.  For example Ebba (yellow line in Wallander girl names Sweden). However, this influence is not as big as we hypothesized.

*  There are also evidence that some names decreases in popularity as series comes out. For example William(pink line in Skam boys names Norway).
